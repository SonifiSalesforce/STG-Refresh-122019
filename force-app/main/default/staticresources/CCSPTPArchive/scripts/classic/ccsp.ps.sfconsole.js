            ...
            try {
                if (callData.CallInfo) {
                    if (callData.CallInfo.Caller) {
                        from = callData.CallInfo.Caller.Name || callDataHelper.getCallerName(callData);
                    }
                }
                if (!from && callData.CallMessageInfo) {
                    from = callData.CallMessageInfo.CallerName;
                }
                if (!from && callData.CallInfo && callData.CallInfo.EmailInfo && callData.CallInfo.EmailInfo.From) {
                    from = callData.CallInfo.EmailInfo.From.Address;
                }
            } catch (e) {}
            var callLabel = "from: " + (from || 'unknown');
