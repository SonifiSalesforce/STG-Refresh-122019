...
CCSPIntegration.sforceCti.config = (function() {
	...
	function getConfiguration(prefix, request, retrieve, restore) {
		...
		if (configRetrievalTimeout > 0) {
			configRetrievalTimer = window.setTimeout(
				function() {
					d.reject("Timed out in waiting response from Salesforce. Check if path to softphone in call center settings is exactly the same as of integration.js");
				},
	...
	function getUserConfiguration() {
		...
		var request = function(callback) {
			sforce.interaction.runApex('AccountRetrievalNoSpacename', 'getUser', '', callback);
		}
