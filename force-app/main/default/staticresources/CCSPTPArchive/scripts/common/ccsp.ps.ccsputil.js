...
function __formatCallerNumber(dataValue) {
    try {
        if (!dataValue)
            return dataValue;

        var config = CCSPIntegration[sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti'].config.callcenterConfig();

        if (!config ||
            !config.customOptions ||
            !config.customOptions.regexpToRemoveFromCallerNumbers) {
            log.info('Regexp to format not found on path CCSPIntegration[sfLoad_.isLightning ? \'sfLghtCti\' : \'sforceCti\'].config.callcenterConfig(), skipping formatting.');
            return dataValue;
        }

        var regexpToFormat = new RegExp(config.customOptions.regexpToRemoveFromCallerNumbers);

        log.info('Preparing to format caller number: ' + dataValue + ' removing by regexp ' + regexpToFormat);

        return dataValue.replace(regexpToFormat, "");
    } catch (e) {
        log.error('Error during __formatCallerNumber: ' + e);
        return dataValue
    }
}
...
(function(target) {
    ...
    target.getCallerANI = function(callData) {
        if (!callData) return "";
        var ani = getProperty(callData, "CallerANI") || getValueByKey(callData.CallInfo.Caller, "ANI") || "";
        return __formatCallerNumber(ani);
    };
    ...
    target.getCallerDNIS = function(callData) {
        if (!callData) return "";
        var dnis = getProperty(callData, "CallerDNIS") || getValueByKey(callData.CallInfo.Caller, "DNIS") || "";
        return __formatCallerNumber(dnis);
    };
    ...
    target.getOptionalParameter = function(callData, name) {
        ...
        target.extractDialerRecord = function(screenPopInfo, columnNameMapping, mappingColumnsOnly) {
            log.info("Extracting dialer record: screenPopInfo: " + screenPopInfo + ", columnNameMapping: " + columnNameMapping + ", mappingColumnsOnly: " + mappingColumnsOnly);
            ...
            var popData = [],
                pdColumns, columnsCount, keyval, keyvalPattern;
            ...
            log.info("extractDialerRecord: returning: " + JSON.stringify(popData));
            return popData;
        };
    ...
    target.getDialerRecord = function(callData, useColumnNameMapping, mappingColumnsOnly) {
        log.info("getDialerRecord: starting: " + callData);
        if (!callData) return [];
...
(function(target) {
    ...
    function searchAndGetPop(searchParam, queryParam, searchActionName, defer, isMultiple) {

        var getPopIdForClassic = function(popId, data, isMultiple) {
            if (isMultiple) {
                return data["screenPopUrl"];
            } else {
                for (var i in data) {
                    if (i !== "screenPopUrl") {
                        return "/" + i;
        ...
        var doneFunc = function(searchActionName, data, defer) {
            var popId;
            try {
                log.debug(searchActionName + ": result: " + JSON.stringify(data));

                if (!sfLoad_.isLightning) {
                    popId = getPopIdForClassic(popId, data, isMultiple);
                    if (popId) {
                        defer.resolve(popId);
                    } else {
                    ...
                    var screenPopObject = data && data["SCREEN_POP_DATA"];

                    if (screenPopObject) {
                        defer.resolve(screenPopObject);
                    } else {
                        defer.reject(GET_SCREENPOP_NO_SEARCH_RESULT);
                    }
                }
                ...
                log.error(searchActionName + ".getScreenPop: done: error: " + (e.message || e));
                defer.reject(GET_SCREENPOP_FAILED);
            }
        };

        var failFunc = function(searchActionName, error, defer) {
            log.error(searchActionName + ".getScreenPop error: " + error);
            defer.reject(GET_SCREENPOP_FAILED);
        };

        if (sfLoad_.isLightning) {
            CCSPIntegration.sfLghtCti.api.searchAndGetScreenPopUrl(searchParam, queryParam || "", sforce.opencti.CALL_TYPE.INBOUND)
                .done(function(data) { doneFunc(searchActionName, data, defer); })
                .fail(function(error) { failFunc(searchActionName, error, defer); });
        } else {
            CCSPIntegration.sforceCti.api.searchAndGetScreenPopUrl(searchParam, queryParam, "inbound")
                .done(function(data) { doneFunc(searchActionName, data, defer); })
                .fail(function(error) { failFunc(searchActionName, error, defer); });
        }
    }
    ...
    CallerIdSearchAction.prototype.getScreenPop = function(callData, dialerRecord) {
        log.info("CallerIdSearchAction.prototype.getScreenPop: enter:  callData:" + JSON.stringify(callData) + " , dialerRecord:" + JSON.stringify(dialerRecord));
        var defer = $.Deferred();
        var callerId;
        if (dialerRecord) {
            callerId = callDataHelper.getDialerDataByName(dialerRecord, "DialedTelephoneNumber");
            callerId = __formatCallerNumber(callerId);
        } else {
        ...
        if (!callerId) {
            try {
                if (callData.CallInfo) {
                    if (callData.CallInfo.Caller) {
                        callerId = callData.CallInfo.Caller.Name || callDataHelper.getCallerName(callData);
                    }
                }
                if (!callerId && callData.CallMessageInfo) {
                    callerId = callData.CallMessageInfo.CallerName;
                }
                if (!callerId && callData.CallInfo && callData.CallInfo.EmailInfo && callData.CallInfo.EmailInfo.From) {
                    callerId = callData.CallInfo.EmailInfo.From.Address;
                }
            } catch (e) {}
        }
        ...
        if (!callerId) {
            ...
            searchAndGetPop(callerId, "", "CallerIdSearchAction", defer, isMultiple);
    ...
    ObjectIdSearchAction.prototype.getScreenPop = function(callData, dialerRecord) {
        log.info("ObjectIdSearchAction.prototype.getScreenPop: enter:  callData:" + JSON.stringify(callData) + " , dialerRecord:" + JSON.stringify(dialerRecord));
        var defer = $.Deferred();
        var objectId, popId;
        if (dialerRecord) {
    ...
    QuerySearchAction.prototype.getScreenPop = function(callData, dialerRecord) {
        log.info("QuerySearchAction.prototype.getScreenPop: enter:  callData:" + JSON.stringify(callData) + " , dialerRecord:" + JSON.stringify(dialerRecord));
        var defer = $.Deferred();
        ...
        if (!searchParamValue || queryParams === null) {
            ...
            var isMultiple = this.isMultiple;
            searchAndGetPop(searchParamValue, queryParams, "QuerySearchAction", defer, isMultiple);
        }
    ...
    ApexSearchAction.prototype.getScreenPop = function(callData, dialerRecord) {
        log.info("ApexSearchAction.prototype.getScreenPop: enter:  callData:" + JSON.stringify(callData) + " , dialerRecord:" + JSON.stringify(dialerRecord));
        var defer = $.Deferred();
        ...
        if (!methodParams) {
            ...
            CCSPIntegration[sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti'].api.runApex(this.apexClass, this.methodName, methodParams)
                .done(function(result) {
                    try {
                        log.debug("ApexSearchAction: result: " + JSON.stringify(result));
                        var item, popId;
                        if (sfLoad_.isLightning && result.runApex) {
                            result = JSON.parse(result.runApex);
                        }
                        if (result.length > 0) {
                ...
                if (customOptions.hasOwnProperty(searchOptionsKey)) {
                    ...
                    for (var index = 0; index < searchOptions.length; ++index) {
                        ...
                        if (customOptions.hasOwnProperty(optionKey)) {
                            ...
                            log.warn("loadSearchOptions: key missing: " + optionKey);
                        }
                    }
                }
            }
