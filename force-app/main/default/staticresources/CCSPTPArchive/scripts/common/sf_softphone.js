...
var screenPopManager = (function() {
    ...
    function screenPop(callId, popId, force) {
        ...
        if (sfLoad_.isLightning) {
            var screenPopObject = {};
            if (Object.prototype.toString.call(popId) === "[object String]") {
                screenPopObject = {
                    type: sforce.opencti.SCREENPOP_TYPE.URL,
                    params: { url: popId }
                };
            } else {
                screenPopObject = popId;
            }
            CCSPIntegration.sfLghtCti.api.screenPop(screenPopObject)
    ...
    function getPopIdFromPageInfo(pageInfo) {
        var id = pageInfo.recordId || pageInfo.objectId;

        if (id) {
            log.debug("screenPopManager.getPopIdFromPageInfo: get popId from recordId: " + id);
            return "/" + id;
        }
...
function isInConsole() {
    try {
        return CCSPIntegration[sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti'].console.wrapper.isInConsole();
    } catch (e) {
        return false;
    }
}
...
function screenPopCallData(callType, direction, callData, callInfo, dialerRecord) {
    var callId = callInfo.callId || callDataHelper.getCallId(callData);
    CCSPIntegration.ccsp.sfPopHelper.getScreenPop(callType, direction, callData)
        .done(function(popId) {
...
function setTouchPointURL(url) {
    $("#touchPointFrame").find('iframe').attr('src', url ? url : '');
}

function hideTabs() {
    $("#touchPointFrame").find('iframe').attr('src', '');
    $(".psTabs").hide();
}

function onLoadDoneOrFail() {
    translator.onLoad();
    $('.loaderContainer').hide();
    $('#first-tab-group, .psWrapper').removeClass('sf-hidden');
}


// called when salesforce configuration has loaded successfully.
CCSPIntegration[sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti'].onLoadDone = function(cc, user) {
    ...
    $("#agent-extension-number").text('(' + user.Extension + ')');
    ...
    if (cc.customOptions) {
        ...
        if (!isInConsole() && !sfLoad_.isLightning) {
            hideTabs();
        }
...
CCSPIntegration[sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti'].onLoadFail = function(error) {
    displayAgentExtension("config error: " + error);
    onLoadDoneOrFail();
}
 ...
 */
CCSPIntegration[sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti'].onClickToDial = function(dialData) {
    try {
        log.debug("CCSPIntegration" + (sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti') + ".onClickToDial: " + dialData.number);
...
CCSPIntegration.sfhub.events.CallData = function(lineId, eventName, callInfo, callData) {
    ...
    var lastCallInfo = lastCallInfoStorage.restore();
    if (lastCallInfo && lastCallInfo.callId !== callId) {
        log.error('Additional call received during active call. ' +
            ' This is not supported in the current release, allowing 2 simultaneous calls in TouchPoint can lead to unexpected behaviour in SalesForce. ');
        return;
    }
    ...
    if (isInConsole())
        CCSPIntegration[sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti'].console.wrapper.fireOnCallBegin(callInfo, callData);
...
CCSPIntegration.sfhub.events.CallStateChange = function(lineId, callInfo) {
    ...
    if (callInfo.mediaType === "VOIP" && callInfo.state === "OnHold") {
        CCSPIntegration[sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti'].api.getPageInfo()
            .done(function(data) {
                try {
...
CCSPIntegration.sfhub.events.WrapUpData = function(lineId, callInfo, callData, wrapupData, userData) {
    ...
    function createOrUpdateTask(id) {
        ...
        CCSPIntegration[sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti'].api.getPageInfo()
            .done(function(pageInfo) {
                ...
                if (!sfLoad_.isLightning)
                    saveParams = $.param(saveParams);

                log.debug("WrapUpData : saveParams " + JSON.stringify(saveParams));

                CCSPIntegration[sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti'].api.saveLog('Task', saveParams)
                    .done(function(result) {
                        log.debug("WrapUpData: saveLog: succeeded. result: " + JSON.stringify(result));
                    })
                    .fail(function(error) {
                        $('#saveLogAlert').modal();
                        var saveLogId = parseInt(Math.random() * 10000000000);
                        var row = $('<tr></tr>');
                        row.attr('id', 'saveLogRow-' + saveLogId);
                        var col = $('<td></td>').addClass('saveLog-cell');
                        var saveLogButton = $('<a></a>');
                        saveLogButton.addClass('styled-button ui-link');
                        saveLogButton.text(translator.translateStaticText('saveLogBtn') + (callInfo.from || callData.Caller.ANI || callData.Caller));
                        saveLogButton.on('click', function() {
                            CCSPIntegration.sfhub.events.WrapUpData(lineId, callInfo, callData, wrapupData, userData);
                            $('#saveLogRow-' + saveLogId).remove();
                        });

                        $('.opt-table tbody').append(row.append(col.append(saveLogButton)));
    ...
    if (isInConsole()) {
        var createTask = true;
        CCSPIntegration[sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti'].console.wrapper.fireOnCallEnd(callInfo, callData, wrapupData);
        CCSPIntegration[sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti'].console.wrapper.onCallLogSaved(function(result) {
            if (!createTask)
...
CCSPIntegration.sfhub.events.RequestedAgentInfoResponse = function(resultCode, requestId, agentInfo) {
    ...
    if (agentInfo.lines.length > 0) {
        var lines = agentInfo.lines,
            lineId, screenPopCallId = screenPopManager.getScreenPopCallId();
        ...
        log.debug("RequestedAgentInfoResponse: no lines found for agent, clearing");
        displayCallInfo(null, null);
    }
    if (!sendCallDataRequest) {
        updateCallDetailLinkVisibility();
    }
    ...
    if (currentCallInfo && currentCallInfo.state === "SlaveA2AInCall" && currentCallInfo.transferreeAgentGlobalId) {
        ...
        CCSPIntegration[sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti'].api.getPageInfo()
            .done(function(data) {
                ...
                if (!data.objectId && !data.recordId /* This means, that this is page not related to any object (like contacts page) and can't be transferred.*/) {
                    $('#syncAlert').modal();
                    return;
                }
...
$(function() {
    ...
    updateCallDetailLinkVisibility();
    CCSPIntegration[sfLoad_.isLightning ? 'sfLghtCti' : 'sforceCti'].config.load(true);
});